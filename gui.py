...
def save_str_values():
    ...
    try:
        ...
        save_json()
    create_and_display_structs()
    create_alg_window()


def create_and_display_structs():
    global g, trucks_list, orders_lst
    g, trucks_list, orders_lst = src.algorithm.create_structures(variables['structures_data']["rows_cols"], 
...
def validate_data_and_append_alg(checked_value, algorithm_vars):
    ...
    if algorithm_vars in integer_alg_vars:
        try:
            int_value = int(checked_value)
            if int_value - float(checked_value) != 0:
